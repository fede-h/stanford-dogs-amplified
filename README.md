...
![Pipeline structure](assets/project_structure.png)

### 1. Download the Base Dataset
`load_dataset.py` downloads the base Stanford Dogs dataset. This builds the fundamental folder structure (`data/stanford_dogs/...`) and extracts `images.tar` and `annotation.tar`.
```bash
python src/load_dataset.py
```

### 2. Generate Classes CSV
`get_classes.py` parses the Annotations directory to catalog every breed and their respective ImageNet ID prefix into a CSV file. `bbid.py` uses this CSV to know what to search for.
```bash
python src/get_classes.py
```
...
`bbid.py` downloads new images from Bing based on the CSV. The script requests specific search terms (e.g., `'[breed] dog photo'`), standardizes the downloads into strict RGB JPGs (resolving alpha channels and `webp`/`avif` formats), and places them into categorized folders (e.g., `data/bing/n02085620-Chihuahua`).
...
`yolo_clean.py` processes the newly downloaded bulk images. `yolo_clean.py` loops through `data/bing/` and infers bounding boxes using YOLO26m object detection model.
